...
#include <mco/ap/assignment_generator.h>
...
shared_ptr<AssignmentInstance> AssignmentGenerator::create_instance() {

	auto graph = make_shared<Graph>();
	auto weights = make_shared<EdgeArray<Point *>>(*graph);
	list<node> agents;
	...
	auto agent_set = make_shared<set<node>>(agents.cbegin(), agents.cend());
	auto assignment_instance = make_shared<AssignmentInstance>(graph, weights, agent_set, objective_generator_.dimension());
