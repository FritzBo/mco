...
APBensonDualSolver::APBensonDualSolver(std::shared_ptr<AssignmentInstance> inpt_instance, double epsilon):
...
APBensonDualSolver::~APBensonDualSolver() {
}

void APBensonDualSolver::augment(node v) {
	while(label_[v] != v) {
...
double APBensonDualSolver::Solve_scalarization(Point& weights, Point& value) {
...
bool APBensonDualSolver::check_equality_subgraph(Point &cost, Point &dual1, Point &dual2, double epsilon_) {
...
bool APBensonDualSolver::slack_at_least_zero(Point &cost, Point &dual1, Point &dual2, double epsilon_) {
	for(unsigned int i = 0; i < cost.dimension(); ++i)
...
bool APBensonDualSolver::better_slack(Point &slack, Point &cost, Point &dual1, Point &dual2, double epsilon_) {
	for(unsigned int i = 0; i < cost.dimension(); ++i)
