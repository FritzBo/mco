...
bool EpSolverBSPc::check_heuristic_prunable(const Label& label) {
...
void EpSolverBSPc::Solve(const ForwardStar& graph,
...
//		cout << current_node << ": ";
        ...
        if(current_node != source && !parent_check(current_node, node_entries, graph))
        {
            continue;
        }
...
bool EpSolverBSPc::parent_check(node current_node,
                              FSNodeArray<NodeEntry>& node_entries,
                              const ForwardStar& graph)
{

//    for(auto label : node_entries[current_node].new_labels())
//    {
...
//    }
    ...
    for(auto label : node_entries[current_node].labels())
    {
        if(label->pred_label == nullptr)
        {
            return false;
...
//    FSNodeArray<vector<unsigned>*> label_ids(graph, nullptr);
//
//    for(auto& label : node_entries[current_node].new_labels())
//    {
//        node pred_node = graph.tail(label.pred_edge);
//
//        if(label_ids[pred_node] == nullptr)
//        {
//            label_ids[pred_node] = new vector<unsigned>;
//            for(auto& check_label : node_entries[pred_node].labels())
//            {
//                label_ids[pred_node]->push_back(check_label.label_id);
//            }
//            for(auto& check_label : node_entries[pred_node].new_labels())
//            {
//                label_ids[pred_node]->push_back(check_label.label_id);
//            }
//            std::sort(label_ids[pred_node]->begin(),
//                      label_ids[pred_node]->end());
//        }
//
//        bool pred_exists = std::binary_search(label_ids[pred_node]->begin(),
//                                              label_ids[pred_node]->end(),
//                                              label.pred_label_id);
//
//        if(!pred_exists)
...
//        for(auto& check_label : node_entries[pred_node].labels())
//        {
//            if(check_label.label_id == label.pred_label_id)
//            {
//                pred_exists = true;
//                break;
//            }
