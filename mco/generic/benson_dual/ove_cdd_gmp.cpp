...
OnlineVertexEnumeratorCddGmp::OnlineVertexEnumeratorCddGmp(Point &initial_value, unsigned int dimension, double epsilon) :
	...
	for(unsigned int i = 0; i < dimension_ - 1; ++i) {
		for(unsigned int j = 0; j < dimension_; ++j)
			dd_set_si(h_representation_->matrix[i][j + 1], i == j ? 1 : 0);
		dd_set_si(h_representation_->matrix[i][0], 0);
	...
	for(unsigned int j = 0; j < dimension_ - 1; ++j)
		dd_set_si(h_representation_->matrix[dimension_ - 1][j + 1], -1);
	dd_set_si(h_representation_->matrix[dimension_ - 1][dimension_], 0);
	dd_set_si(h_representation_->matrix[dimension_ - 1][0], 1);

	for(unsigned int j = 0; j < dimension_ - 1; ++j) {
		dd_set_si2(h_representation_->matrix[dimension_][j + 1], round((initial_value[j] - initial_value[dimension_ - 1]) * den),
                   den);
    }

	dd_set_si(h_representation_->matrix[dimension_][dimension_], -1);
	dd_set_si2(h_representation_->matrix[dimension_][0],
               round(initial_value[dimension_ - 1] * den),
               den);
...
OnlineVertexEnumeratorCddGmp::~OnlineVertexEnumeratorCddGmp() {
...
bool OnlineVertexEnumeratorCddGmp::has_next() {
	return !unprocessed_vertices_.empty();
}

Point * OnlineVertexEnumeratorCddGmp::next_vertex() {
...
void OnlineVertexEnumeratorCddGmp::add_hyperplane(Point &vertex, Point &normal, double rhs) {
	clock_t start = clock();
...
//	std::cout << "rhs: " << rhs << std::endl;
	...
	for(unsigned int i = 0; i < dimension_; ++i)
		dd_set_si2(new_equality->matrix[0][i + 1],
                   round(normal[i] * den),
                   den);
	dd_set_si2(new_equality->matrix[0][0],
               -round(rhs * den),
               den);
